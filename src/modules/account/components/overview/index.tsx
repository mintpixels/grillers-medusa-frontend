...
const Overview = ({
  ...
  const orderCount = (orders?.length || 0) + legacyOrderCount
  const recentOrders = buildRecentOrders(orders, legacyOrders)
        ...
        <ul data-testid="orders-wrapper">
          {recentOrders.length > 0 ? (
            recentOrders.map((recent) => (
              <li
                key={`${recent.source}-${recent.order.id}`}
                data-testid="order-wrapper"
                data-value={recent.order.id}
                className="border-b border-gray-50 last:border-0"
              >
                {recent.source === "medusa" ? (
                  <LocalizedClientLink
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span
                          ...
                          data-value={recent.order.display_id}
                        >
                          Order #{recent.order.display_id}
                        </span>
                        <OrderStatusBadge
                          status={recent.order.fulfillment_status}
                        />
                      </div>
                      <p
                      ...
                      >
                        {new Date(recent.order.created_at).toLocaleDateString(
                          "en-US",
                          {
                            month: "short",
                            day: "numeric",
                    ...
                    >
                      {convertToLocale({
                        amount: recent.order.total,
                        currency_code: recent.order.currency_code,
                      })}
                    </span>
                  ...
                  <LocalizedClientLink
                    ...
                    className="flex items-center gap-4 px-6 py-4 hover:bg-gray-50/50 transition-colors"
                  >
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-SilverPlate text-xs font-maison-neue-mono text-Charcoal/55">
                      Past
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="truncate text-sm font-maison-neue font-semibold text-Charcoal">
                          Invoice {legacyOrderDisplayId(recent.order)}
                        </span>
                      ...
                      <p className="truncate text-xs font-maison-neue text-Charcoal/50 mt-0.5">
                        {recent.order.placed_at
                          ? new Date(recent.order.placed_at).toLocaleDateString(
                              "en-US",
                              {
                                month: "short",
                                day: "numeric",
                          ...
                          : "Unknown date"}
                        {" · "}
                        {legacyLineTitle(recent.order.lines[0])}
                      </p>
                    </div>
            ...
            <li className="px-6 py-12 text-center">
              <svg
                className="w-12 h-12 mx-auto text-Charcoal/20 mb-3"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={1}
              ...
              <p
                className="text-sm font-maison-neue text-Charcoal/50 mb-4"
...
function OrderStatusBadge({ status }: { status?: string }) {
  ...
  return (
    <span
      className={`inline-flex px-2 py-0.5 text-[10px] font-maison-neue font-semibold rounded-full border ${config.className}`}
    >
      {config.label}
    </span>
  )
